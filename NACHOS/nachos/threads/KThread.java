...
public class KThread {
	...
	public void join() {
		/*
		 * Machine.interrupt().disable(); Lib.debug(dbgThread,
		 * currentThread.name + " is Joining to thread: " + toString());
		 * Lib.assertTrue(this != currentThread); if(this.status ==
		 * statusFinished) //if targeted thread is terminated return; //do
		 * nothing threadsJoinedOnMe.add(currentThread); //add self to targets
		 * dependencies sleep();
		 */
	...
	public static void selfTest() {
		...
		// test case 1
		/*
		 * KThread thread1 = new KThread(new PingTest(2)).setName(
		 * "forked thread 1"); KThread thread2 = new KThread(new
		 * PingTest(3)).setName("forked thread 2"); thread1.fork();
		 * thread2.fork(); //fork two threads, and call join on one of them
		 * System.out.println("Start Join"); thread1.join();
		 * 
		 * System.out.println("Join Complete"); //so it shouldn't run until the
		 * other one completes new PingTest(1).run();
		 * 
		 * System.out.println("******************");
		 */
		 ...
		 * thread1.fork();
		 * 
		 * System.out.println("START Join"); thread1.join(); //main sleeps until
		 * thread 1 is done System.out.println("Join Complete");
		 * 
		 * System.out.println("START Join"); thread1.join();
		 * 
		 ...
		 * KThread thread1 = new KThread(new PingTest(2)).setName(
		 * "forked thread 1"); KThread thread2 = new KThread(new
		 * PingTest(2)).setName("forked thread 1"); thread1.fork(); //main forks
		 * this System.out.println("START Join"); thread1.join(); //main sleeps
		 * System.out.println("Join Complete"); thread2.fork(); //thread1 forks
		 * this System.out.println("START Join"); thread2.join(); //thread1
		 * sleeps System.out.println("Join Complete");
		 */
