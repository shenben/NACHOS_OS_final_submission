...
int main(serverAddress){
	...
	create a socket connected to port 15
	while (true){
		read user input into incomingMessage, putting it after existing data up to the end of the buffer
		if incomingMessage has data 
			while incomingMessage has a newline {
				// newlineIndex points to the location of the newline char
				memcpy(temp,incomingMessage, newlineIndex)
				if temp is '.'
					close s
